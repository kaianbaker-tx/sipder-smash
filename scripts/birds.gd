...
## zooms through them. One draw call for every bird.
...
func _ready() -> void:
	...
	for f in 6:
		...
		flocks.append({"centre": centre, "r": rng.randf_range(14, 26), "speed": rng.randf_range(0.25, 0.45), "phase": rng.randf() * TAU, "scatter": 0.0})
	batch = SpriteBatch.new()
	add_child(batch)
	batch.setup(TEX, Vector2(1.9, 0.95), flocks.size() * PER)
...
func _process(delta: float) -> void:
	...
	for f in flocks:
		...
		for i in PER:
			...
			var p := c + Vector3(cos(a) * r, sin(_t * 1.3 + i * 0.8) * 2.0 + i * 0.4 + fl.scatter * 3.0, sin(a) * r)
			batch.put(idx, p, Vector2(1.0, 0.4 + 0.6 * absf(sin(_t * 9.0 + i))))
			idx += 1
