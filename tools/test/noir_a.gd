...
func build() -> void:
	...
	for bx in 4:
		for bz in 4:
			...
			for k in 4:
				var qv := [Vector2(-1, -1), Vector2(1, -1), Vector2(-1, 1), Vector2(1, 1)][k] as Vector2
...
func _process(delta: float) -> void:
	...
	# lightning: flash the sky, then thunder
	_flash_t -= delta
	if _flash_t <= 0.0:
		_flash_t = rng.randf_range(6.0, 11.0)
		RenderingServer.global_shader_parameter_set("sky_horizon", Color(1, 1, 1))
		RenderingServer.global_shader_parameter_set("sky_mid", Color(0.9, 0.9, 0.95))
		var tw := create_tween()
		tw.tween_interval(0.07)
		tw.tween_callback(func() -> void:
			RenderingServer.global_shader_parameter_set("sky_horizon", palette.sky_horizon)
			RenderingServer.global_shader_parameter_set("sky_mid", palette.sky_mid))
		tw.tween_interval(0.08)
		tw.tween_callback(func() -> void: RenderingServer.global_shader_parameter_set("sky_horizon", Color(0.9, 0.9, 0.95)))
		tw.tween_interval(0.05)
		tw.tween_callback(func() -> void: RenderingServer.global_shader_parameter_set("sky_horizon", palette.sky_horizon))
		tw.tween_interval(0.4)
		tw.tween_callback(func() -> void: Sfx.play("thunder", 0.15))
